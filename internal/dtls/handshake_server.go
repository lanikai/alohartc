...
func (hs *serverHandshakeState) setCipherSuite(id uint16, supportedCipherSuites []uint16, version uint16) bool {
	for _, supported := range supportedCipherSuites {
		if id == supported {
			...
			if version < VersionDTLS12 && candidate.flags&suiteTLS12 != 0 {
				continue
			}
			hs.suite = candidate
			return true
...
var suppVersArray = [...]uint16{VersionDTLS10, VersionDTLS12}
...
func (hs *serverHandshakeState) clientHelloInfo() *ClientHelloInfo {
	...
	supportedVersions = suppVersArray[VersionDTLS12-hs.clientHello.vers:]
