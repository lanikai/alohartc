...
func (m *clientHelloMsg) equal(i interface{}) bool {
	...
	return bytes.Equal(m.raw, m1.raw) &&
		...
		eqUint16s(m.protectionProfiles, m1.protectionProfiles) &&
		bytes.Equal(m.mki, m1.mki)
}
...
func (m *clientHelloMsg) marshal() []byte {
	...
	if m.protectionProfiles != nil {
		extensionsLength += 2 + len(m.protectionProfiles)*2 + 1 + len(m.mki)
		numExtensions++
	}
	if numExtensions > 0 {
	...
	if m.protectionProfiles != nil {
		...
		length := 2 + len(m.protectionProfiles)*2 + 1 + len(m.mki)
		z[2] = byte(length >> 8)
		z[3] = byte(length)
		z[4] = uint8(len(m.protectionProfiles) >> 7)
		z[5] = uint8(len(m.protectionProfiles) << 1)
		for i, profile := range m.protectionProfiles {
			z[6+i*2] = uint8(profile >> 8)
			z[7+i*2] = uint8(profile)
		}
		z[6+len(m.protectionProfiles)*2] = uint8(len(m.mki))
		copy(z[6+len(m.protectionProfiles)*2+1:], m.mki)
		z = z[6+len(m.protectionProfiles)*2+1+len(m.mki):]
	}
...
func (m *clientHelloMsg) unmarshal(data []byte) bool {
	...
	m.raw = data
	m.seq = uint16(data[4])<<8 | uint16(data[5])
	...
	cookieLen := int(data[47+sessionIdLen])
	if len(data) < 47+sessionIdLen+cookieLen {
		return false
	}
	m.cookie = data[47+sessionIdLen : 47+sessionIdLen+cookieLen]
	data = data[47+sessionIdLen+cookieLen:]
	if len(data) < 2 {
		return false
	}
...
func (m *helloVerifyRequestMsg) marshal() (x []byte) {
	if m.raw != nil {
		return m.raw
	}

	length := 15 + len(m.cookie)

	x = make([]byte, 15+len(m.cookie))
...
func (m *certificateStatusMsg) marshal() []byte {
	...
	if m.statusType == statusTypeOCSP {
		x = make([]byte, 12+4+len(m.response))
...
func (m *certificateStatusMsg) unmarshal(data []byte) bool {
	...
	if m.statusType == statusTypeOCSP {
		...
		m.seq = uint16(data[4])<<8 | uint16(data[5])
		respLen := uint32(data[13])<<16 | uint32(data[14])<<8 | uint32(data[15])
...
func (m *newSessionTicketMsg) marshal() (x []byte) {
	...
	x[10] = uint8(length >> 8)
	x[11] = uint8(length)
	x[16] = uint8(ticketLen >> 8)
	x[17] = uint8(ticketLen)
	copy(x[18:], m.ticket)
...
func (m *newSessionTicketMsg) unmarshal(data []byte) bool {
	...
	ticketLen := int(data[16])<<8 + int(data[17])
	if len(data)-18 != ticketLen {
		return false
	}
