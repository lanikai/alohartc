...
func makeClientHello(config *Config) (*clientHelloMsg, error) {
	...
	hello := &clientHelloMsg{
		...
		protectionProfiles:           config.ProtectionProfiles,
	}
...
func (hs *clientHandshakeState) handshake() error {
	...
	hs.messageSequence = hs.hello.seq + 1
...
func (hs *clientHandshakeState) doFullHandshake() error {
	...
	if certRequested {
		...
		certMsg.seq = hs.messageSequence
		hs.messageSequence += 1
		hs.finishedHash.Write(certMsg.marshal())
	...
	if chainToSend != nil && len(chainToSend.Certificate) > 0 {
		...
		certVerify.seq = hs.messageSequence
		hs.messageSequence += 1
...
func (hs *clientHandshakeState) sendFinished(out []byte) error {
	...
	if hs.serverHello.nextProtoNeg {
		...
		nextProto.seq = hs.messageSequence
		hs.messageSequence += 1
	...
	finished.seq = hs.messageSequence
	hs.messageSequence += 1
